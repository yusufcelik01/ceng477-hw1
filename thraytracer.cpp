...
int main(int argc, char* argv[])
{
    ...
    for(cam_id = 0; cam_id < num_of_cameras; cam_id++){
        ...
        // tharg[2].start = tharg[1].end = 2*height;
        // tharg[3].start = tharg[2].end = 3*height;
...
void foo(int thno){
    ...
    for(int y=tharg[thno].start; y < tharg[thno].end; y++){
        for(int x=0; x < arg.width; x++){
            ...
            parser::Vec3f temp_vec = (x*arg.pixel_width+arg.p_width)* arg.u;
            ...
            pixel_color = getRayColor(*(arg.scene), *(arg.normals_of_meshes), r, arg.scene->max_recursion_depth, true);

            if(pixel_color == NULL)//meaning ray didn't hit any objects
            {
                pixel_color = new parser::Vec3f;
                pixel_color->x = arg.scene->background_color.x;
                pixel_color->y = arg.scene->background_color.y;
                pixel_color->z = arg.scene->background_color.z;
            }
